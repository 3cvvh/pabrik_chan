...
<div class="min-h-screen bg-gray-100 py-8 px-2 sm:px-6 lg:px-8">
    <div class="max-w-3xl mx-auto animate-fadeIn">
        ...
        <div class="mb-6 transform transition-all duration-500 animate-slideDown">
            <h2 class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-gray-900">Tambah Stok Baru</h2>
        ...
        <div class="bg-white shadow-2xl rounded-2xl overflow-hidden transform transition-all duration-500 hover:shadow-3xl animate-slideUp">
            <div class="p-4 sm:p-8">
                <form action="{{ route('crud_stocks.store') }}" id="create-form" method="post" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        ...
                        <div class="transform transition-all duration-300 animate-fadeIn" style="animation-delay: 100ms">
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <input value="{{ old('jumlah') }}" type="number" name="jumlah" id="nama" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-3 px-4 text-sm md:text-base border-gray-300 rounded-lg" placeholder="Masukkan jumlah stok">
                            </div>
                            @error('jumlah')
                        ...
                        <div class="transform transition-all duration-300 animate-fadeIn" style="animation-delay: 100ms">
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <input value="{{ old('jumlah') }}" type="date" name="tanggal_masuk" id="nama" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-3 px-4 text-sm md:text-base border-gray-300 rounded-lg" placeholder="Masukkan jumlah stok">
                            </div>
                            @error('tanggal_masuk')
                            <p class="mt-2 text-sm md:text-base text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="transform transition-all duration-300 animate-fadeIn col-span-1 md:col-span-2" style="animation-delay: 300ms">
                            ...
                            <div class="mt-1">
                                <textarea id="keterangan" name="keterangan" rows="3" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-3 px-4 text-sm md:text-base border-gray-300 rounded-lg" placeholder="Masukkan keterangan stok">{{ old('keterangan') }}</textarea>
                            </div>
                            @error('deskripsi')
                            <p class="mt-2 text-sm md:text-base text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="transform transition-all duration-300 animate-fadeIn" style="animation-delay: 600ms">
                            <label for="id_produk" class="block text-sm md:text-base font-semibold text-gray-700">Produk</label>
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <select name="id_produk" id="id_produk" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-3 px-4 text-sm md:text-base border-gray-300 rounded-lg">
                    ...
                    <div class="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-4 pt-6 border-t animate-fadeIn" style="animation-delay: 700ms">
                        <a href="{{ route('crud_stocks.index') }}" class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-3 border border-gray-300 shadow-sm text-sm md:text-base font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            Kembali
                        </a>

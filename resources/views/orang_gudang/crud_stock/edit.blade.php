                ...
                <form action="{{ route('crud_stocks.update',$stock->id) }}" id="create-form" method="post" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        ...
                        <div class="transform transition-all duration-300 animate-fadeIn" style="animation-delay: 100ms">
                            <label for="jumlah" class="block text-sm md:text-base font-semibold text-gray-700">Jumlah</label>
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <input value="{{ old('jumlah',$stock->jumlah) }}" type="number" name="jumlah" id="jumlah" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-3 px-4 text-sm md:text-base border-gray-300 rounded-lg" placeholder="Masukkan jumlah stok">
                            </div>
                            @error('jumlah')
                        ...
                        <div class="transform transition-all duration-300 animate-fadeIn" style="animation-delay: 200ms">
                            <label for="tanggal_masuk" class="block text-sm md:text-base font-semibold text-gray-700">Tanggal Masuk</label>
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <input value="{{ old('tanggal_masuk',$stock->tanggal_masuk) }}" type="date" name="tanggal_masuk" id="tanggal_masuk" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-3 px-4 text-sm md:text-base border-gray-300 rounded-lg">
                            </div>
                            @error('tanggal_masuk')
                            <p class="mt-2 text-sm md:text-base text-red-600">{{ $message }}</p>
                            @enderror

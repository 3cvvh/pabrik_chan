...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100">
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-8 animate-fade-in">
            ...
            <!-- Desktop Button -->
            <a href="{{ route('crud_gudang.create') }}"
            ...
            @foreach ($data as $index => $g)
                <div class="p-4 bg-white border border-gray-200 rounded-xl shadow-md hover:shadow-lg transition-all duration-200">
                    ...
                    <div class="mb-1 flex flex-col gap-1">
                        <div>
                            <span class="block text-xs text-gray-500">Pabrik</span>
                            <span class="block text-sm font-semibold text-gray-800">{{ $g->pabrik->name ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="block text-xs text-gray-500">Nama Gudang</span>
                            <span class="block text-sm font-semibold text-gray-800">{{ $g->nama }}</span>
                        </div>
                        <div>
                            <span class="block text-xs text-gray-500">Alamat</span>
                            <span class="block text-sm text-gray-700">{{ $g->alamat }}</span>
                        </div>
                        <div>
                            <span class="block text-xs text-gray-500">No Telepon</span>
                            <span class="block text-sm text-gray-700">{{ $g->no_telepon }}</span>
                        </div>
                        <div>
                            <span class="block text-xs text-gray-500">Keterangan</span>
                            <span class="block text-sm text-gray-500 italic">
                                {{ $g->keterangan ? $g->keterangan : 'Tidak ada keterangan' }}
                            </span>
                        </div>
                    </div>
                    <!-- Tombol Aksi -->

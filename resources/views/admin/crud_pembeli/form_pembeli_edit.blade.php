...
<div class="min-h-screen bg-gray-100 py-8 px-2 sm:px-6 lg:px-8">
    <div class="max-w-3xl mx-auto animate-fadeIn">
        ...
        <div class="mb-6 transform transition-all duration-500 animate-slideDown">
            <h2 class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-gray-900">Edit Data Pembeli</h2>
            <p class="mt-1 text-sm md:text-base text-gray-600">Silahkan edit data pembeli dengan lengkap</p>
        </div>
        ...
        <div class="bg-white shadow-2xl rounded-2xl overflow-hidden transform transition-all duration-500 hover:shadow-3xl animate-slideUp">
            <div class="p-4 sm:p-8">
                <form id="edit-form" action="/dashboard/admin/crud_pembeli/{{ $pembeli->id }}" method="post" class="space-y-8">
                    @csrf
                    @method('put')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        ...
                        <div class="transform transition-all duration-300 animate-fadeIn" style="animation-delay: 200ms">
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <input type="text" name="name" id="name" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-3 px-4 text-sm md:text-base border-gray-300 rounded-lg" placeholder="Masukkan nama pembeli" value="{{ $pembeli->name }}">
                            </div>
                            @error('name')
                                <p class="mt-2 text-sm md:text-base text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="transform transition-all duration-300 animate-fadeIn" style="animation-delay: 300ms">
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <input type="text" name="alamat" id="alamat" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-3 px-4 text-sm md:text-base border-gray-300 rounded-lg" placeholder="Masukkan alamat pembeli" value="{{ $pembeli->alamat }}">
                            </div>
                            @error('alamat')
                                <p class="mt-2 text-sm md:text-base text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="transform transition-all duration-300 animate-fadeIn" style="animation-delay: 400ms">
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <input type="number" name="no_telepon" id="no_telepon" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-3 px-4 text-sm md:text-base border-gray-300 rounded-lg" placeholder="Masukkan nomor telepon" value="{{ $pembeli->no_telepon }}">
                            </div>
                            @error('no_telepon')
                                <p class="mt-2 text-sm md:text-base text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-4 pt-6 border-t animate-fadeIn" style="animation-delay: 700ms">
                        ...
                        <button onclick="confirmEditPembeli(this)" type="button" class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-3 text-sm md:text-base border border-transparent font-medium rounded-lg shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            Update Data
                        </button>
                    </div>
                </form>
...
<script>
function confirmEditPembeli(button) {
    Swal.fire({
        title: 'Apakah anda yakin?',
        text: "akan memperbarui data pembeli",
        icon: 'warning',

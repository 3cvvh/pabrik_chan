...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 animate-fade-in">
            ...
            <a href="{{ route('crud_pembeli.create') }}" class="mt-4 sm:mt-0 w-full sm:w-auto">
        ...
        <!-- Table (Desktop) -->
        <div class="hidden sm:block bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-fade-in-up" style="animation-delay: 0.2s" id="tableContainer">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">No Telepon</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Aksi</th>
                    </tr>
                </thead>
                <tbody id="pembeli-tbody" class="bg-white divide-y divide-gray-200">
                    @foreach ($data as $index => $pembeli)
                    <tr class="hover:bg-gray-50 transition-all duration-200">
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $data->firstItem() + $index }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-800">{{ $pembeli->name }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $pembeli->pabrik->name ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $pembeli->alamat }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $pembeli->no_telepon }}</td>
                        <td class="px-6 py-4">
                            <div class="flex space-x-2">
                                <a href="{{ route('crud_pembeli.edit',$pembeli->id) }}" class="inline-flex items-center px-3 py-1.5 bg-indigo-100 text-indigo-700 hover:bg-indigo-200 rounded-lg text-sm font-medium transition-all duration-200 hover:scale-105">
                                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                    Edit
                                </a>
                                <form action="{{ route('crud_pembeli.destroy',$pembeli->id) }}" method="POST" class="delete-form" style="display:inline;">
                                    @csrf @method('DELETE')
        ...
        <!-- Cards (Mobile) -->
        <div id="pembeli-cards" class="sm:hidden space-y-4 mt-6">
            @foreach ($data as $index => $pembeli)
            <div class="p-4 bg-white border border-gray-200 rounded-lg shadow hover:shadow-md transition">
                <div class="flex justify-between mb-2">
                    ...
                    <span class="px-2 py-1 text-xs font-medium rounded-full bg-indigo-100 text-indigo-800">{{ $pembeli->pabrik->name ?? '-' }}</span>
                </div>
...
(function () {
    ...
    const tbody = document.getElementById('pembeli-tbody');
    const cards = document.getElementById('pembeli-cards');
    ...
    async function fetchAndReplace(url, push = true) {
        try {
            ...
            const newCards = doc.getElementById('pembeli-cards');
            ...
            if (newCards && cards) {
                cards.innerHTML = newCards.innerHTML;
            }
            if (newPagination) {
    ...
    function rebindPaginationLinks() {
        ...
        // Penting: bind ulang tombol delete setelah pagination/search
